...
### 10. Deep Auth Flow Overhaul & Edge-Case Bugs (Agent Session — 2026-08-26)
- **Bug 1 (Register Redirect):** Users were thrown an unhandled 403 error on registration because the backend had Email Verification enabled (preventing auto-login). We updated `RegisterForm.jsx` to catch this gracefully, toast an instruction to check their email, and redirect to `/login`.
- **Bug 2 (Username Auth):** The login form offered a "Username" field, but Supabase/InsForge SDK natively only supports email/phone. We created a Postgres `SECURITY DEFINER` function `get_email_by_username` to securely resolve usernames to emails behind the scenes before calling `signInWithPassword`.
- **Bug 3 (The Blank Screen `rt.from` crash):** A brittle polyfill in `api.js` (`if (insforge.database) ...`) failed to initialize fast enough, causing a fatal `rt.from is not a function` error that crashed the entire React app. We replaced it with a robust `Object.defineProperty` getter.
- **Bug 4 (OAuth Callbacks):** Google/GitHub logins were failing silently because `AuthCallback.jsx` attempted to fetch the profile before the InsForge SDK had finished exchanging the URL token for a session. We implemented a robust exponential backoff retry system in the callback handler.
- **Bug 5 (Redirect Loop on Onboarding):** After completing the onboarding steps, users were redirected straight back to `/onboarding` instead of `/dashboard`. 
  - **Root Cause:** The InsForge SDK's `setProfile` endpoint was responding with a `406 Not Acceptable` (or returning `null` data) in the background. Because of this silent drop, our metadata extraction logic resulted in an empty object, wiping the `onboarding_completed: true` flag from the local React state just before the redirect.
  - **Resolution:** Updated `updateProfile` in `auth.service.js` to implement a smart merge fallback. If the SDK returns a null payload, it now manually merges the known fields (`metadataFields`) directly into the existing context payload. This securely preserved the flag and resolved the infinite redirect loop.
